...
public class PluginLoader {
    ...
    protected PluginClassLoader loadClassLoader(PluginYAML pluginConfig, File pluginJar) {
        try {
            ...
            logger.error("Error while creating class loader(plugin={})", pluginConfig.getName());
        }
        return null;
    }
    ...
    protected Plugin loadPluginJAR(PluginYAML pluginConfig, File pluginJar, PluginClassLoader loader) {
        try {
            ...
            logger.error("Error while loading plugin main class(main={}, plugin={})", pluginConfig.getMain(), pluginConfig.getName(), e);
        }
        return null;
    }
    ...
    protected PluginYAML loadPluginData(File file, Yaml yaml) {
        try (JarFile pluginJar = new JarFile(file)) {
            ...
            if (configEntry == null) {
                logger.warning("Jar file {} doesnt contain a waterdog.yml or plugin.yml!", file.getName());
                return null;
            }
            ...
            logger.warning("Invalid plugin.yml for {}: main and/or name property missing", file.getName());
        } catch (Exception e) {
            logger.error("Can not load plugin files in {}", file.getPath(), e);
        }
        return null;
    }
}
