...
public class ServerInfoMap {
    public static ServerInfoType getDefaultType() {
    ...
    public ServerInfo get(String address, int port) {
        for (ServerInfo info : this.serverList.values()) {
            if (info.getAddress().getHostString().equalsIgnoreCase(address) && info.getAddress().getPort() == port) {
                return info;
            }
        }
        return null;
